...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5548988062654:+55 48988062654\n' // WhatsApp ID + phone number
            + 'END:VCARD'
/******END OF VCARD INPUT******/
		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] O BOT PRECISA SER ADM DO GRUPOA❌',
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5548991635455@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'criador':
                  client.sendMessage(from, {displayname: "Joao", vcaurd: vcaurd}, MessageType.contact, { quoted: mek})
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Cade o codigo da linguagem', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo ademiro @${nomor.split("@s.whatsapp.net")[0]}\agora adms podem falar `,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo ademar @${sender.split("@")[0]}\agora *os membros comum* podem falar `,
					contextInfo: { mentionedJid: [sender] }
					}
            ...
            break
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativar o modo simi com sucesso ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativando o modo simi com sucesso ✔️')
					} else {
